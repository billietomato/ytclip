...
这套工作流可以帮你抓直播、找出最值得剪的片段、加快剪辑流程、把字幕精准对齐到成片上，再翻译成其他语言，让推的话语带着爱、热情和效率传到更远的地方。
...
第 2 步和第 5 步会用到 AI 代理，帮你找出值得剪的片段并处理翻译。这份 README 以 Anthropic 的 **Claude Code** 为例。
...
这一步分两段：先把字幕切成区块，再让 AI 判断哪些片段最适合做成粉丝向切片。
...
> 读取 `~/my-video/chunks.json`，按照 `ytclip-2-highlight-moments/references/highlight-evaluation-rubric.md` 中的评分标准，找出前 10-20 个最适合做成 VTuber 粉丝向切片的片段。结果保存为 `~/my-video/highlight-moments.md`。
...
第 2 步中的 AI 会从 8 个维度评估字幕分块，筛出最值得剪、翻、分享的片段：
