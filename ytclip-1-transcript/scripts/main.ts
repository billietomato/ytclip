...
function normalizeLanguageCode(code: string): string {
  ...
  if (lowered === "zh-tw" || lowered === "zh_hant" || lowered === "zh-hant-tw") return "zh-Hant";
  if (lowered === "zh-cn" || lowered === "zh_hans") return "zh-Hans";
  return c;
}
...
function findTranscript(
  ...
  for (const lang of languages) {
    const found = filtered.find((t) => t.languageCode === normalizeLanguageCode(lang));
...
async function fetchTranscriptSnippets(info: TranscriptInfo, translateTo?: string): Promise<{ snippets: Snippet[]; language: string; languageCode: string }> {
  ...
  if (translateTo) {
    const normalized = normalizeLanguageCode(translateTo);
    if (!info.isTranslatable) throw new Error(`Transcript ${info.languageCode} is not translatable`);
    const tl = info.translationLanguages.find((t) => t.languageCode === normalized);
    if (!tl) throw new Error(`Translation language ${normalized} not available`);
    url += `&tlang=${normalized}`;
    lang = tl.language;
    langCode = normalized;
  }
...
async function processVideo(videoId: string, opts: Options): Promise<VideoResult> {
  ...
  if (!needsFetch && videoDir) {
    ...
    const wantLangs = opts.translate ? [normalizeLanguageCode(opts.translate)] : opts.languages.map(normalizeLanguageCode);
...
Options:
  ...
  --translate <code>           Translate to language code (e.g. zh-Hant, zh-Hans)
...
function parseArgs(argv: string[]): Options | null {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "-h" || arg === "--help") {
      ...
      const v = argv[++i];
      if (v) opts.languages = v.split(",").map((s) => normalizeLanguageCode(s.trim()));
    } else if (arg === "--format") {
      ...
      opts.translate = normalizeLanguageCode(argv[++i] || "");
