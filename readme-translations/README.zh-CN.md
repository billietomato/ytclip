...
第 2 步、第 5 步和第 7 步会用到 AI 代理，帮你找出有趣片段、翻译和校对。这份 README 以 Anthropic 的 **Claude Code** 为例。
...
> **使用其他 AI 代理？** 只要你的 AI 代理看得懂第 2 步、第 5 步和第 7 步用到的 `SKILL.md` 和参考文档，也能照样完成。
...
└───────────────────────────────┘
    ...
    ▼
┌───────────────────────────────┐
│ 第 6 步：手动调整字幕            │  你的视频编辑软件
└───────────────────────────────┘
    │  微调后的繁中 SRT
    ▼
┌───────────────────────────────┐
│ 第 7 步：校对繁中字幕            │  ytclip-5-proofread-zhtw (AI)
└───────────────────────────────┘
    │  校对后的 SRT
    ▼
┌───────────────────────────────┐
...
─── 第三阶段：简中转换 & 导出简中版 ─────────────
    │
┌───────────────────────────────┐
│ 第 9 步：转简中字幕              │  ytclip-6-convert-tc-to-sc
└───────────────────────────────┘
    │  简体中文 SRT
    ▼
┌───────────────────────────────┐
...
把翻译好的繁中 SRT 导入视频编辑软件，对照视频逐行检查字幕：
...
> 这一步是品质把关的关键。先把时间轴、用字和断句都调到满意，再把最终 SRT 导出给 AI 做最后一次错字检查。
...
让 AI 校对你手动调整后导出的最终繁中 SRT，只抓真正的错字，不会给你风格建议。打开 Claude Code 输入：

> Use ytclip-5-proofread-zhtw skill. 校对 `my-video/transcript-zhtw-final.srt`，只列出确定的错字（错别字、乱码、明显打错的字）。

AI 会报告每一条错字的 SRT 区块编号、原文和正确写法。如果没有错字，就会告诉你文件是干净的。校对完毕后，先把错字修正回 `my-video/transcript-zhtw-final.srt`，再重新导入视频编辑软件，替换原本的繁中字幕轨道。
